...
{
    public partial class frmOrdinance : Form
    {
        SqlConnection cn;
        SqlCommand cm;
        SqlDataReader dr;
        frmOrdinanceList f;
        public frmOrdinance(frmOrdinanceList f)
        {
            InitializeComponent();
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    cn.Open();
                    cm = new SqlCommand("insert into tblOrdinance (ordinance) VALUES (@ordinance)", cn);
                    cm.Parameters.AddWithValue("@ordinance", txtOrdinance.Text);
                    cm.ExecuteNonQuery();
                    cn.Close();
